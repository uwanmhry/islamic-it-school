...
function renderBookDetail(book) {
    ...
    // Create order buttons based on available links and ref parameter
    const orderButtonsHtml = `
        <div class="mt-4 flex flex-col gap-3">
            ${ref ? `
                <a 
                    href="#" 
                    class="wa-button bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg text-center font-semibold transition duration-200" 
    ...
    const navigationButtonsHtml = `
        <div class="mt-8 flex gap-4">
            ${prevBook ? `
                <button id="prevBookBtn" class="bg-gray-100 hover:bg-gray-200 text-gray-800 font-semibold py-2 px-6 rounded transition duration-200 flex items-center">
            ...
            ${nextBook ? `
                <button id="nextBookBtn" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-6 rounded transition duration-200 flex items-center">
                    Berikutnya
    ...
    if (nextBook) {
        ...
        if (nextBtn) {
            nextBtn.addEventListener('click', () => {
                window.location.href = `detailbuku.html?id=${nextBook.id}${ref ? `&ref=${ref}` : ''}`;
            });
        }
    }
    ...
    if (prevBook) {
        ...
        if (prevBtn) {
            prevBtn.addEventListener('click', () => {
                window.location.href = `detailbuku.html?id=${prevBook.id}${ref ? `&ref=${ref}` : ''}`;
            });
        }
    }

    // Add event listeners for WhatsApp buttons (if ref parameter exists)
    if (ref) {
